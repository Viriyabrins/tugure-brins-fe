...
import { PAYMENT_STATUS_OPTIONS, formatCurrency } from "../utils/paymentStatusConstants";
import { usePaymentStatusData } from "../hooks/usePaymentStatusData";
    ...
    const columns = [
        ...
        { header: "Period", accessorKey: "period" },
        {
            ...
            cell: (row) => (
                <span className="font-semibold">
                    IDR {formatCurrency(row.total_amount || 0)}
                </span>
            ),
        },
        {
            ...
            cell: (row) => (
                <span className="text-green-600 font-medium">
                    IDR {formatCurrency(row.paid_amount || 0)}
                </span>
            ),
        },
        {
            ...
            cell: (row) => (
                <span className="text-orange-600 font-medium">
                    IDR {formatCurrency(row.outstanding_amount || 0)}
                </span>
            ),
        },
        ...
        },
        {
            ...
            cell: (row) => {
                ...
                const matched = invoicePayments.filter(
                    (p) => p.match_status === "MATCHED",
                ).length;
                if (matched === invoicePayments.length)
                    return <StatusBadge status="MATCHED" />;
                if (matched > 0)
                    return <StatusBadge status="PARTIALLY_MATCHED" />;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <StatCard
                    ...
                    subtitle={
                        overdueInvoices > 0 ? "Requires attention" : "All current"
                    }
                    icon={AlertTriangle}
                    gradient
                    className={
                ...
                <Card>
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <CardTitle>
                                Invoice Detail - {selectedInvoice.invoice_number}
                            </CardTitle>
                            <Button
                                variant="outline"
                                size="sm"
                        ...
                        <div className="mt-6">
                            ...
                            {getInvoicePayments(selectedInvoice.id).length === 0 ? (
                                ...
                                <div className="space-y-2">
                                    {getInvoicePayments(selectedInvoice.id).map(
                                        (payment, index) => (
                                            <div
                                                key={index}
