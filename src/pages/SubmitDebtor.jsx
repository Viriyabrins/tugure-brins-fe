...
export default function SubmitDebtor() {
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      if (batchMode === "new") {
        ...
        // Auto-create Bordero for this batch
        const now = new Date();
      ...
      for (let i = 0; i < rows.length; i++) {
        try {
          ...
          const payload = {
            ...
            nomor_rekening_pinjaman: row.nomor_rekening_pinjaman?.trim() || "",
            nomor_peserta: row.nomor_peserta?.trim() || "",
            loan_type: row.loan_type?.trim() || "",
            loan_type_desc: row.loan_type_desc?.trim() || "",
            cif_rekening_pinjaman: row.cif_rekening_pinjaman?.trim() || "",
            jenis_pengajuan_desc: row.jenis_pengajuan_desc?.trim() || "",
            jenis_covering_desc: row.jenis_covering_desc?.trim() || "",
            tanggal_mulai_covering: parseDate(row.tanggal_mulai_covering),
            tanggal_akhir_covering: parseDate(row.tanggal_akhir_covering),
            ...
            nomor_perjanjian_kredit: row.nomor_perjanjian_kredit?.trim() || "",
      ...
      // Create Bordero AFTER debtor creation to use actual uploaded count
      if (batchMode === "new" && borderoId) {
        try {
          await backend.create("Bordero", {
            ...
            batch_id: batchId,
            period: period,
            total_debtors: uploaded, // Use actual uploaded count
            total_exposure: totalExposure,
            total_premium: totalPremium,
            currency: "IDR",
            status: "GENERATED",
          ...
          console.error("Failed to create Bordero:", borderoError);
        }
      }
      ...
      try {
        await backend.create("AuditLog", {
          action: batchMode === "new" ? "BULK_UPLOAD" : "BULK_REVISION",
          module: "DEBTOR",
          entity_type: "Debtor",
          entity_id: batchId,
          old_value: "",
        ...
        for (const oldDebtor of revisionDebtorsInBatch) {
          ...
          try {
            ...
            await backend.create("ReviseLog", {
              ...
              nominal_premi: parseFloat(oldDebtor.nominal_premi) || null,
  ...
  const filteredDebtors = debtors.filter((debtor) => {
    ...
    const searchMatch =
      !filters.name ||
      debtor.nama_peserta?.toLowerCase().includes(filters.name.toLowerCase()) ||
  ...
  const columns = [
    ...
    },
    {
      ...
      accessorKey: "plafon",
      cell: (row) => (
        <div className="font-medium">{formatRupiahAdaptive(row.plafon)}</div>
      ),
    },
    {
      header: "Net Premi",
      ...
      cell: (row) => (
        <div className="font-medium">{formatRupiahAdaptive(row.net_premi)}</div>
      ),
    },
    {
      header: "Status",
        ...
        {selectedDebtors.length >= 0 && (
          <>
            ...
            >
              ...
              Revision{" "}
              {selectedDebtors.length > 0 ? `(${selectedDebtors.length})` : ""}
            </Button>
            {/* <Button>
                            variant="outline"
                            onClick={}
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <Alert>
              ...
              <AlertDescription>
                Download the template first to see the required format. Make
            ...
            {batchMode === "revise" && (
              <Alert className="border-orange-200 bg-orange-50">
                ...
                <AlertDescription className="text-orange-800">
                  ...
                  must match a REVISION debtor in the selected batch. Old
                  REVISION data will be moved to ReviseLog after upload.
                </AlertDescription>
              </Alert>
            )}
          </div>
