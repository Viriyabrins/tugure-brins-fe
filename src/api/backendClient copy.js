...
export const backend = {
  ...
  async processMasterContractWorkflowAction(contractId, payload) {
    ...
    const endpoint = `/api/apps/${encodeURIComponent(appId)}/master-contracts/${encodeURIComponent(contractId)}/workflow-action`;
    const res = await fetch(url, await authFetchOptions({
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
  ...
  async processClaimWorkflowAction(claimNo, payload) {
    ...
    const endpoint = `/api/apps/${encodeURIComponent(appId)}/claims/${encodeURIComponent(claimNo)}/workflow-action`;
    const res = await fetch(url, await authFetchOptions({
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
 ...
 /**
  ...
  async getUsersByRole(realm, roleName) {
    const requestId = `FBAPI_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const url = `/api/users-by-role/${encodeURIComponent(realm)}/${encodeURIComponent(roleName)}`;
    
    console.log(`[${requestId}][BackendAPI.getUsersByRole] ════════════════════════════════════════`);
    console.log(`[${requestId}][BackendAPI.getUsersByRole] Request Initiated`);
    console.log(`[${requestId}][BackendAPI.getUsersByRole] Realm: "${realm}", Role: "${roleName}"`);
    console.log(`[${requestId}][BackendAPI.getUsersByRole] URL: ${url}`);
    
    const fetchOptions = await authFetchOptions({}, url);
    console.log(`[${requestId}][BackendAPI.getUsersByRole] Fetch Options:`, {
      headers: {
        ...fetchOptions.headers,
        Authorization: fetchOptions.headers?.Authorization ? 
          `${fetchOptions.headers.Authorization.substring(0, 30)}...` : 'NOT SET',
      },
      method: fetchOptions.method || 'GET',
    });
    
    console.log(`[${requestId}][BackendAPI.getUsersByRole] Sending request...`);
    const res = await fetch(url, fetchOptions);
    
    console.log(`[${requestId}][BackendAPI.getUsersByRole] Response Status: ${res.status} ${res.statusText}`);
    
    if (!res.ok) {
      console.error(`[${requestId}][BackendAPI.getUsersByRole] ✗ Request failed with status ${res.status}`);
      try {
        await throwBackendError(res);
      } catch (err) {
        console.error(`[${requestId}][BackendAPI.getUsersByRole] Error thrown:`, err.message);
        console.log(`[${requestId}][BackendAPI.getUsersByRole] ════════════════════════════════════════\n`);
        throw err;
      }
    }
    ...
    const text = await res.text();
    console.log(`[${requestId}][BackendAPI.getUsersByRole] Response Body (first 200 chars):`, text.substring(0, 200));
    
    try {
      const parsed = JSON.parse(text);
      let result = [];
      if (parsed?.success === true) result = parsed.data || [];
      else if (Array.isArray(parsed)) result = parsed;
      else if (parsed?.data && Array.isArray(parsed.data)) result = parsed.data;
      
      console.log(`[${requestId}][BackendAPI.getUsersByRole] ✓ Parsed result: ${result.length} user(s)`);
      result.forEach((user, idx) => {
        console.log(`[${requestId}][BackendAPI.getUsersByRole]   [${idx + 1}] ${user.email}`);
      });
      console.log(`[${requestId}][BackendAPI.getUsersByRole] ════════════════════════════════════════\n`);
      return result;
    } catch (e) {
      console.error('[BackendAPI.getUsersByRole] Failed to parse response:', e);
      console.log(`[${requestId}][BackendAPI.getUsersByRole] ════════════════════════════════════════\n`);
      return [];
    }
  },
